...
rayon::rayon(QString nom_produit,int id_rayon,int etat)
{
...
bool rayon::ajouter()
{
    ...
    query.prepare("INSERT INTO RAYON (NOM_PRODUIT, ETAT , ID_RAYON) "
                        "VALUES (:nom_produit, :etat, :id_rayon)");
...
model->setHeaderData(2, Qt::Horizontal, QObject::tr("id_rayon "));
return model;
...
void rayon::modifier(QString nom_produit,int id_rayon,int etat)
{
    QSqlQuery query;

    query.prepare("update RAYON SET NOM_PRODUIT = ?,ETAT = ? where ID_RAYON = ?");
    query.addBindValue(nom_produit);
    query.addBindValue(QString::number(etat));
    query.addBindValue(QString::number(id_rayon));
    query.exec();
